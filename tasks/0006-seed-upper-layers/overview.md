...
| [005](005.md) | Upper-layer seed endpoints | [x] | Add POST endpoints for strategic-objectives, tactical-objectives, and policies seeding. Each validates prerequisites, clears existing items, gathers context, runs the seed agent, and commits to git. Includes integration tests. |
| [006](006.md) | Admin page seed card redesign | [x] | Redesign the seed status card as a vertical layer list showing all 5 layers with status indicators, item counts, and per-layer seed buttons. Disable upper-layer buttons when prerequisites are empty. Add i18n keys. Update TypeScript types for new API shape. |
| [007](007.md) | E2E tests | [x] | Add Playwright tests for the redesigned seed card: 5-layer display, disabled button ordering, loading states, Full Cascade button presence. |
